...
1. Создайте каталог для ключей и токенов. Он исключён из Git:

   ```bash
   mkdir -p secrets
   ```

2. В Google Cloud включите Google Sheets API, создайте service account и получите
...
3. Скопируйте [.env.example](.env.example) в `.env` и заполните значения в редакторе.
...
4. Загрузите настройки и проверьте одну среду:
   ...
   Имя среды должно присутствовать и в копии реестра со статусом `active` или
   `configuring`, и среди файлов `internal/gitlab/testdata/*.json`, откуда stub
   берёт образы. Stub всё равно читает Jira и Sheets. Ожидаемый лог:
   `релиз определён`, затем `dry-run: запись пропущена`. Предупреждение Jira
   означает, что тестовые теги не подтверждены вашими задачами.
5. Для сбора реальных образов задайте в `.env` параметры вашего collect-images,
